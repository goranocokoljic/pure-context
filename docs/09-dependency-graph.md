...
- **Package-style imports in languages without a resolver** (e.g., PHP namespaces, Rust crate paths, Haskell modules) — these are currently indistinguishable from external packages, so repos in those languages have few or zero edges. Graph tools built on `dep_edges` return empty results there; that is a missing graph, not an empty dependency set. See the support matrix in [LANGUAGE-SUPPORT.md](../LANGUAGE-SUPPORT.md#which-languages-get-dependency-edges).
