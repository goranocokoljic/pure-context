...
- **Version:** 1.37.0. Phases 1–104 are COMPLETE (latest: Phase 104 — Test-Mapper Redesign: one tokenizing pass per test file stored in `test_file_tokens` (schema v15, additive), incremental by self-detection (hash + anti-join; `index_file` keeps it current), lazy build on first tool use (`coverage: 'built on demand'`), `check_index_staleness.testMapper`. Legacy `NAME` semantics kept byte-for-byte (oracle in `test/core/oracles/`, delete in 1.38); one deliberate change: every test file is scanned, not only those with symbols. jenkins 35.7 s → 1.9 s, saleor 111.6 s → 1.7 s, no-op 0 ms. Findings: `dev-docs/in-progress/phase104-findings.md`).
- **Next:** Phase 105 Search-Quality Sweep 3 (`dev-docs/PHASE105_TASKS.md`, 1.38.0; the elixir `defp` penalty strength is a candidate lever). Also due in 1.38: delete the legacy mapper oracle `test/core/oracles/test-mapper-legacy.ts`.
- **Re-index note:** none forced since 1.31.0; test-mapper token rows + mapping build on each repo's next whole-tree run or first coverage-needing tool call (1.37.0); Ruby/Swift edges and Elixir/C++ private symbols appear on each repo's next whole-tree run (1.36.0); symbol refs backfill on each repo's next whole-tree run (`list_repos.symbolRefs` = 0 until then); content migrates to the blob store the same way; cross-index DI edges appear on an android root's next run.
