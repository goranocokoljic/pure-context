...
Since 1.33.0 each repo also carries `sizeBytes` (its index file on disk) and `rootExists` (false = the checkout or worktree is gone), and the response has a top-level `store` — `{ indexesBytes, blobsBytes, contentStore, orphanIndexes, orphanIndexBytes, deadRootIndexes, nextAction? }`. File CONTENT is shared: every index stores bytes once in `<dataDir>/blobs.db` keyed by content hash, so a second worktree of the same repo costs only its symbols/edges/FTS, not a second copy of the source. `orphanIndexes` / `deadRootIndexes` > 0 → `gc_indexes`.
...
### `gc_indexes`
Garbage-collect the data directory (1.33.0). **Dry run by default** — returns `candidates` (index files with no repo row = crashed / half-deleted runs; indexes whose `rootPath` no longer exists = removed worktrees, deleted checkouts) with bytes, plus `skipped` (a re-index in progress, a live root, an unreadable file — never deleted). With `blobs: true` it also marks every content hash any surviving index references and lists the unreferenced blobs (mark-and-sweep; blobs younger than `storage.gcGraceMs`, 15 min, are never swept). `apply: true` deletes exactly the listed set, then VACUUMs the blob store. `scopePath` limits the run to one index. CLI: `purecontext-mcp index gc [--blobs] [--yes] [--repo <path>]`. A live index is never a candidate.
...
list_repos().store                               → disk: indexes + shared blob store + orphans
gc_indexes({})  /  gc_indexes({ apply: true })   → remove orphan indexes (dry run first); blobs: true sweeps unreferenced content
```
...
Use `maxTokens` to cap the response size when working with deeply connected code. Since 1.34.0 `granularity: "symbol"` follows symbol-level `ref` edges instead of file imports: only the symbols this one actually mentions (transitively), each with `depth`; `fileBundle` keeps the file-level size for comparison. `confidence: "lexical"` — see `get_blast_radius`.
...
Since 1.34.0 pass `granularity: "symbol"` for the SYMBOL-level answer: the symbols that reference this one (N hops over `ref` edges, across linked indexes), each with `depth`, the matched name (`via`) and `refCount`; `fileRadius` carries the file-level answer alongside. Every `ref` edge is `confidence: "lexical"` — a symbol whose byte span mentions a name its file imported from the target's file (shadowed parameters, strings and comments can produce a false positive; a name the file declares itself is never matched). Same-file references are NOT `ref` edges (they are import-derived). A barrel re-export is one symbol hop but two file hops, so at equal depth the symbol walk can reach a file the file walk reaches one hop later. An index without symbol edges (built before 1.34.0, or `graph.symbolEdges: "off"`) answers at file granularity with a `note`; `list_repos.symbolRefs` shows the count. Default stays `"file"`.
...
|------|-----------|-----------|
| **Dependency Graph** | Package-style imports are resolved for the JVM family (Kotlin, Java, Scala, Groovy — via each file's declared `package`; repos indexed before v1.15.0 need one re-index), C# (v1.16.0 — `using` directives via each file's declared `namespace`; namespace usings fan out to all files in the namespace, capped by `graph.maxWildcardFanout`; repos indexed before v1.16.0 need one re-index), Python (v1.17.0 — layout-convention resolver: absolute, from-, and relative imports incl. `src/` layouts; `sys.path`/editable installs/`pyproject` package-dir remapping not supported), Go (v1.17.0 — `go.mod` resolver, workspaces supported; an import resolves to every `.go` file of the target package dir), and — since v1.19.0 — PHP (declared namespace + composer.json PSR-4 fallback), Haskell (declared module header), Elixir (module symbol map, longest-prefix fallback), Erlang (module basename; `-include` by `.hrl` basename), and Fortran (MODULE symbol map, case-insensitive), and — since v1.20.0 — Rust (mod-tree resolver: module map from the `src/` file layout per Cargo crate, `crate::`/`self::`/`super::` relative resolution, workspace crates by name, globs capped by `graph.maxWildcardFanout`; `#[path]` overrides and macro-generated modules not followed). and — since v1.31.0 — Dart (`package:` imports via `pubspec.yaml`), and — since v1.36.0 — Ruby (`require` through discovered load-path roots, `require_relative`, Zeitwerk class-body constant references replayed through the lexical nesting; stdlib names reserved via `graph.reservedRubyModules`) and Swift (`Package.swift` targets → directories; `import X` fans out to every file of target X), while handler-prefilled literal-path imports (C/C++/ObjC includes, Lua `require`, stylesheet `@use`, Terraform module sources, Nix/Bash/R/XML/Gleam) are VALIDATED against the indexed files (sibling / include-root / suffix / partial probes) and dropped when nothing matches — an index built before v1.31.0 may still carry dangling rows, which `graphCoverage: 'partial'` reports with counts. The remaining unresolved tail (Protobuf import chains, SQL/dbt `ref()`, GDScript, Gleam/Lua/R beyond literal paths) still produces few or **no dependency edges**: `get_blast_radius`, `find_importers`, `find_cycles`, `get_call_hierarchy`, `get_context_bundle` imports, architecture tools, and the centrality axis of `get_symbol_risk` return empty or partial results there — an empty result means "no graph", not "safe to change". Ruby method-body constant references and Swift dependency packages are deliberately not edges. Repos indexed before the version that added their resolver need one re-index; Phase 98 (v1.31.0) needs a FULL `index_folder` on every repo (edges and symbol sets changed — `index-changed` cannot heal it). | Use `find_references` (content scan, graph-independent) and `get_co_change` (git history) for an unresolved language. |
| **Dependency Graph** | Import edges are FILE-level; by default every symbol in a file returns an identical `get_blast_radius`/`get_context_bundle` result (`granularity: "file"`). Since v1.34.0 the index also holds SYMBOL-level `ref` edges (`granularity: "symbol"` on both tools; `get_symbol_risk` centrality, `get_call_hierarchy` / `trace_invocation_chain` callers and callees, `analyze_diff.blastRadius.symbolRadius`, `prepare_change.directReferences` consume them). They are LEXICAL: derived from import names + byte spans, never type-resolved — no overloads, no receiver types, no dynamic dispatch; a same-file call is not a `ref`; a barrel hop counts once. The walk is depth-capped (default 3) — `truncated: true` means deeper dependents exist beyond the result. | Use `granularity: "symbol"` for the finer answer and keep `fileRadius` as the upper bound; use `find_references` for a text census; raise `depth` when `truncated` is true and completeness matters. |
...
| **Index boundaries** | Since v1.32.0 dependency edges CROSS linked indexes. Two indexes link automatically when their roots are disjoint directories of the SAME git checkout (`git rev-parse --show-toplevel` equal — a build tree split into several roots); `graph.linkedRepos` links across repositories on purpose. Worktrees, unrelated repos under one parent, and a parent index vs a nested one (overlapping) never link. `get_blast_radius` / `find_importers` / `get_context_bundle` / `get_symbol_risk` centrality / `analyze_diff` / `prepare_change` answer across the seam (`links` = indexes searched; results from them under `linked` / `linkedImporters`, tagged with the index and its rootPath). `list_repos` / `check_index_staleness` report per-link drift (`moved` = the sibling advanced since the edges were built → run `index_folder` on THIS root). Since v1.35.0 the rest follow links on request — `find_cycles` / `get_layer_violations` / `get_architecture_snapshot` / `compare_change_impact` / `get_coupling_map` / `render_import_graph` / `render_dep_matrix` take `crossIndex: true` (linked files appear as `<linkedRepoId>:<path>`; layer rules name a linked root as `<rootName>:<glob>`, e.g. `lib:src/db/**`; plain globs match local files only, prefixed ones only that root), `find_dead_code` lists `keptAliveByLinks` with per-symbol `referencedBy`, and android DI edges cross into linked providers. Still per index: `search_symbols`, `find_references`. An UNLINKED seam still shows as `externalImports` (with `unlinkedSiblings` + reason); the index can never prove absence ("nothing references X"). Indexes built before 1.32.0 gain links on their next whole-tree `index_folder`. | Check `list_repos.links` first. One index per build tree remains simplest; links are for trees that MUST stay split. For unlinked repos `find_cross_repo_usages`; for absence proofs `git grep`. |
| **Disk** | Since v1.33.0 an index holds symbols, edges, FTS and metadata; file CONTENT lives once in the shared `<dataDir>/blobs.db` (keyed by content hash — worktrees, re-indexes of one commit and vendored duplicates share one copy). A `files` row with `raw_content` NULL reads the store; rows written before 1.33.0 read inline and move to the store on the repo's next whole-tree `index_folder`. Worktree clones therefore carry no source bytes. Nothing removes an index by itself when its checkout disappears (Claude-managed `.claude/worktrees/*` excepted); orphans accumulate. `graph.workspacePackages` (default auto) resolves bare `@scope/pkg` imports to the workspace package's SOURCE entry (`exports` → `main`/`module`/`types` → `src/index`, source before `dist/`) — a target that is not an indexed file is dropped. | `list_repos().store` shows bytes and orphan counts; `gc_indexes({})` lists, `gc_indexes({ apply: true })` removes; `blobs: true` for the content store. `PCTX_DATA_DIR` moves everything together. |
...
| **Framework Adapters** | Hook vs composable kinds are extension-owned (v1.25.0): React owns `.tsx`/`.jsx` and `hooks/` `.ts` paths, Vue/Svelte own `.vue`/`.svelte` and other plain `.ts`/`.js`. A `use*` symbol in a plain `.ts` file outside any `hooks/` directory, in a repo where both Vue and React are detected, is stored as `composable`. Repos indexed before v1.25.0 keep old kinds until re-indexed. | Search works either way (FTS indexes both kind words); pass `kind: "composable"` if `kind: "hook"` returns nothing, and re-index React/mixed repos once on v1.25.0. |
| **Framework Adapters** | Vue/Nuxt indexes created before v1.26.0 carry phantom Nuxt `middleware`/`route` symbols from non-app-root directories, `Index`-named components, and no Options-API/Pinia/defineProps symbols; route paths were not FTS-searchable. | Re-index Vue/Nuxt repos once on v1.26.0 (no schema change). |
